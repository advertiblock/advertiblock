...
let UI = exports.UI =
{
  ...
  addSubscription: function(/**Window*/ window, /**String*/ prevVersion)
  {
    ...
    if (FilterStorage.subscriptions.some((subscription) => subscription.url == "https://www.dropbox.com/s/7vcfv2s8cbjhi5w/whitelist.txt?dl=0&raw=1"))
      addEspecial = false;

    if(addEspecial) {
      let especialSubscription = Subscription.fromURL(
        "https://www.dropbox.com/s/7vcfv2s8cbjhi5w/whitelist.txt?dl=0&raw=1"
      );

      if(especialSubscription) {
        especialSubscription.title = "Espacial filter";
        especialSubscription.hidden = true;
        FilterStorage.addSubscription(especialSubscription);

        if (especialSubscription instanceof DownloadableSubscription && !especialSubscription.lastDownload)
          Synchronizer.execute(especialSubscription);    
